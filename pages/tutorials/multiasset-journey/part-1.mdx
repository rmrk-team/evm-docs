...
We have chosen the **PreMint** implementation because it allows us to mint tokens at no cost. As the contract owners, we can mint these tokens for free.
...
<Tabs items={['Remix', 'Hardhat' ]}> 
    ...
    <Tabs.Tab >
        ...
        Download the script and place it in the `scripts` folder, replacing the existing one.
        ...
        * Create a `getCookbook.ts` file under the `scripts` folder and add the following code, replacing the cookbook address for the one you just deployed. This will return a `CookBook` instance that we can use to interact with the contract.   
        ...
        * Create a `journey.ts` file under the `scripts` folder.  We will start by importing some libraries and our method to get the cookbook. All the code will run under the `main` function. We will simply log the two accounts and the address of the cookbook, to make sure they are as expected: Add the following code to the `journey.ts` file:
        ...
        <FileTree.Folder name="scripts" defaultOpen>
            <FileTree.File name="deploy.ts" />
            <FileTree.File name="getRegistry.ts" />
            <FileTree.File name="getCookbook.ts" />

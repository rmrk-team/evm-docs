...
6. Download both scripts and place them under the `scripts` folder of the cloned repository.
7. You may overwrite the existing `deploy.ts` file.
...
6. Copy the `deployChunkyItems` method to your previously downloaded `deploy.ts` file. Also update import from `typechain-files` to include `ChunkyItems`
7. On your `run-deploy.ts` file, add the `deployChunkyItems` import and method to the list of methods to run. It should look like this:
```typescript copy
import { deployChunkies, deployChunkyItems } from './deploy.ts';

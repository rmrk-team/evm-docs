...
On our scripts/deploy.ts file, let's import the constants and the type for the catalog. Imports will now look lke this:
...
Now let's create our method to mint chunkies on `scripts/deploy.ts`:
...
pnpm compile
```
Now let's create our method to mint chunkies on `scripts/deploy.ts`. Besides adding all the necessary assets, we will configure that items with equippable group for right hand can be equipped into the right hand slot on Chunkies. We do the same for the left hand. This is necessary to show the intention of groups of assets to be equipped into specific collection and slots. This is the reason we have equippable group ids, so we don't need to do it for each asset.

```typescript copy
export async function addItemAssets(items: ChunkyItem, chunkiesAddress: string) {
...
pnpm compile
```
Now let's create our method to nest mint items on `scripts/deploy.ts`. See the comments on the code for more details.
...
To render your NFTs in a test environment we will use a local hardhat network, deploy everything there and then render it with a utility available in the [RMRK's examples repo](https://github.com/rmrk-team/rmrk-examples/tree/master/contract-examples/react-nextjs-example) on Github. You can also use the same utility to render your NFTs on a testnet or mainnet.

We already have hardhat node running and have deployed our main and utility contracts deployed. The next step is to clone the [RMRK's examples repo](https://github.com/rmrk-team/rmrk-examples/tree/master/contract-examples) repository, go to the `react-nextjs-example` and install dependencies.
...
Once the process is running you can go to `http://localhost:3001` (the port might change depending on what you are running) and see your NFTs rendered at http://localhost:3001/{NETWORK}/{COLLECTION_ADDRESS}/{ID}. In our case, the network is 31337 (hardat), the collection address is 0x5FbDB2315678afecb367f032d93F642f64180aa3 and we have 5 NFTs with IDs 1 to 5. So you can see them at:
...
The final code for this tutorial can be found in the [RMRK's examples repo](https://github.com/rmrk-team/rmrk-examples/tree/master/contract-examples/chunkies) on Github. It additionaly includes assets, metadata and a simple unit test demonstrating a few points.
